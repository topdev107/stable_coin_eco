...
interface AutoDepositConfirmModalProps {
  isOpen: boolean
...
export default function AutoDepositConfirmModal({
  isOpen,
  token,
  baseData,
  onDismiss,
        ...
        <div className='mt-3'>
          {
            Object.values(allTokens).map((onetoken,) => {
              if (token?.address !== onetoken.address) {
                return (
                  <RowBetween className='mt-2'>
                    <Text fontSize='13px'>{`Auto swap with ${onetoken.symbol}`}</Text>
                    <Text fontSize='13px'>{`25%(100 ${token?.symbol})`}</Text>
                  </RowBetween>
                )
              }
              return <></>
